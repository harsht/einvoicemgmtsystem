...
namespace BugNET.Projects
{
	...
	public partial class ProjectSummary : BugNET.UserInterfaceLayer.BasePage
	{
        ...
        {
            lnkRSSIssuesByCategory.NavigateUrl = string.Format("~/Rss.aspx?pid={0}&channel=2",ProjectId);
            lnkRSSIssuesByAssignee.NavigateUrl = string.Format("~/Rss.aspx?pid={0}&channel=6",ProjectId);
            lnkRSSIssuesByStatus.NavigateUrl = string.Format("~/Rss.aspx?pid={0}&channel=3",ProjectId);
            lnkRSSIssuesByMilestone.NavigateUrl = string.Format("~/Rss.aspx?pid={0}&channel=1",ProjectId);
            lnkRSSIssuesByPriority.NavigateUrl = string.Format("~/Rss.aspx?pid={0}&channel=4",ProjectId);
            lnkRSSIssuesByType.NavigateUrl = string.Format("~/Rss.aspx?pid={0}&channel=5",ProjectId);
            //Milestone
            List<IssueCount> lsVersion = Issue.GetIssueMilestoneCountByProject(ProjectId);
            //Status
            List<IssueCount> lsStatus = Issue.GetIssueStatusCountByProject(ProjectId);
            //Priority
            List<IssueCount> lsPriority = Issue.GetIssuePriorityCountByProject(ProjectId);
            //User
            List<IssueCount> lsUser = Issue.GetIssueUserCountByProject(ProjectId);
            //Type
            List<IssueCount> lsType = Issue.GetIssueTypeCountByProject(ProjectId);
		...
		private void rptVersions_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
                ...
                ti.Text = bc.Name;
                if(ti.ImageUrl.Length > 0)
                    ti.ImageUrl = bc.ImageUrl;
            }
            else if (e.Item.ItemType == ListItemType.Footer)
            {
                ((Label)e.Item.FindControl("lblUnscheduledCount")).Text = Issue.GetIssueUnscheduledMilestoneCountByProject(ProjectId).ToString();
            }
		}
		...
		private void rptAssignee_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			...
			{
				((Label)e.Item.FindControl("lblUnassignedCount")).Text = Issue.GetIssueUnassignedCountByProject(ProjectId).ToString();
			}
		}
