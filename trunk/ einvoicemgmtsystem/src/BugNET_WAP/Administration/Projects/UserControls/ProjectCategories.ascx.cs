namespace BugNET.Administration.Projects.UserControls
{
	...
	public partial class ProjectCategories : System.Web.UI.UserControl, IEditProjectControl
	{
        ...
        protected void CategoryValidation_Validate(object sender, ServerValidateEventArgs e)
        {
            ...
            if (Category.GetCategoriesByProjectId(ProjectId).Count > 0)
            {
                e.IsValid = true;
            }
            else
        ...
        protected void OkButton_Click(object sender, EventArgs e)
        {
            ...
            if (OldCategoryId != 0)
            {
                ...
                if (RadioButton2.Checked) //reassign issues to existing category.
                {
                    ...
                    {
                        Message1.ShowErrorMessage("You must select a category to reassign the issues to.");
                        return;
                    }
                    if (OldCategoryId == DropCategory.SelectedValue)
                    {
                        Message1.ShowErrorMessage("You can not reassign the issues to the same category.");
                        return;
                    }
                ...
                if (RadioButton3.Checked)
                {
                    ...
                    {
                        Message1.ShowErrorMessage("You must enter a new category to assign the issues to.");
                        return;
                    }
            ...
            {
                Message1.ShowErrorMessage("You cannot delete the root category");
                return;
            }
        }
