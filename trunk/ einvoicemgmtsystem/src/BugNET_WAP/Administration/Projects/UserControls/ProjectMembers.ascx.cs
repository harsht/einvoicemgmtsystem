namespace BugNET.Administration.Projects.UserControls
{
	...
	public partial class ProjectMembers : System.Web.UI.UserControl, IEditProjectControl
	{
		...
		public void Initialize()
		{
			...
			lstAllUsers.DataSource = ITUser.GetAllAuthorizedUsers();
            ...
            List<ITUser> projectUsers = ITUser.GetUsersByProjectId(ProjectId);
		...
		{
            ddlProjectMembers.DataSource = ITUser.GetUsersByProjectId(ProjectId);
			ddlProjectMembers.DataBind();
			ddlProjectMembers.Items.Insert(0,new ListItem("--select user--","0"));
		...
		protected void AddUser(Object s, EventArgs e) 
		{
            ...
            {
                if (Project.AddUserToProject(item.Value, ProjectId))
                {
		...
		protected void RemoveUser(Object s, EventArgs e) 
		{
            ...
            {
                if (Project.RemoveUserFromProject(item.Value, ProjectId))
                {
		...
		private void ddlProjectMembers_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (ddlProjectMembers.SelectedIndex > 0) 
			{
				...
				lstAllRoles.DataSource = Role.GetRolesByProject(ProjectId);
                lstAllRoles.DataTextField = "Name";
                lstAllRoles.DataValueField = "Id";
				lstAllRoles.DataBind();
 
				// Copy selected users into Selected Users List Box
                List<Role> UserRoles = Role.GetRolesForUser(ddlProjectMembers.SelectedValue, ProjectId);
		...
		private void Button3_Click(object sender, EventArgs e)
		{
			...
			{
                Role.AddUserToRole(ddlProjectMembers.SelectedValue, int.Parse(lstAllRoles.SelectedValue));
		...
		private void Button4_Click(object sender, EventArgs e)
		{
			...
			{
                Role.RemoveUserFromRole(ddlProjectMembers.SelectedValue, Int32.Parse(lstSelectedRoles.SelectedValue));
