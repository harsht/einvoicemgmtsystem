...
namespace BugNET.SubversionHooks
{
    public class IssueTrackerIntegration
    {
        ...
        public void UpdateIssueTrackerFromRevision(string repository, string revision)
        {
            ...
            logger.DebugFormat("Looking for search pattern in revision:{0} and respository:{1}...", revision, repository);

            // get all the matching issue id's
            Regex RegexObj = new Regex("\\[?([A-Za-z]{3}-\\d+)\\]?.+");
            ...
            {             
                ...
                //string issueTrackerComment = string.Format("Subversion Commit - Repository: {4},  <b>REV:{0}</B><hr size=1 /><b>Author:</b>&nbsp;&nbsp;{1}<br /><b>Time:</b>&nbsp;&nbsp;{2}<br /><b>Message:</b><br /><pre style\"font-family:arial;\">{3}</pre>",
                    //revision, author, dateTime, Regex.Replace(logMessage, "([A-Za-z]{3}-(\\d+))", "<a href=\"BugDetail.aspx?bid=$2#top\"><b>$1</b></a>"), GetRepositoryName(repository));
                
                BugNetServices services = new BugNetServices();
                services.CookieContainer = new System.Net.CookieContainer();
                services.Url = Settings.Default.BugNET_SubversionHooks_BugNetServices_BugNetServices;
                try
                {
                    ...
                    foreach (var id in issueIds)
                    {
                        try
                        {
                            ...
                            logger.DebugFormat("\n Revision:{0} Id:{1} Repository:{2} Author:{3} DateTime:{4} LogMessage:{5}", revision, id, GetRepositoryName(repository), author, dateTime, Regex.Replace(logMessage, "([A-Za-z]{3}-(\\d+))", "<a href=\"IssueDetail.aspx?id=$2#top\"><b>$1</b></a>"));
                            bool success = services.CreateNewIssueRevision(int.Parse(revision), id, GetRepositoryName(repository), author, dateTime, Regex.Replace(logMessage, "([A-Za-z]{3}-(\\d+))", "<a href=\"IssueDetail.aspx?id=$2#top\"><b>$1</b></a>"));
                            if (success)
